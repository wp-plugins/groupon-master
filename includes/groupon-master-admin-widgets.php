<?php
if( is_multisite() ) {
	function menu_multi_groupon_admin_widgets(){
	// Create menu
	add_submenu_page( 'groupon-master', 'Widgets', 'Widgets', 'manage_options', 'groupon-master-admin-widgets', 'groupon_master_admin_widgets' );
	}
}
else {
	// Create menu
	function menu_single_groupon_admin_widgets(){
		if ( is_admin() )
		add_submenu_page( 'groupon-master', 'Widgets', 'Widgets', 'manage_options', 'groupon-master-admin-widgets', 'groupon_master_admin_widgets' );
	}
}

function groupon_master_admin_widgets(){
?>
<div class="wrap">
...
<h2><b>&nbsp;Widgets</b></h2>
<?php
if(!class_exists('groupon_master_admin_widgets_table')){
	require_once( dirname( __FILE__ ) . '/groupon-master-admin-widgets-table.php');
}
//Prepare Table of elements
$wp_list_table = new groupon_master_admin_widgets_table();
//Table of elements
...
if( is_multisite() ) {
add_action( 'network_admin_menu', 'menu_multi_groupon_admin_widgets' );
}
else {
add_action( 'admin_menu', 'menu_single_groupon_admin_widgets' );
}
?>